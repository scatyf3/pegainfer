...
| `benchmarks/toolcall-constrained-decoding.md` | BFCL tool-call `tool_choice=required` vs `auto` on vLLM 0.21/Qwen3-0.6B. Three cost channels: grammar compile (TTFT `+60~72%` at c=1, hidden at c=16), per-token mask (TPOT `+9~14%` at c=16), output inflation (`+7~18%` tokens, all concurrencies). Only the first is cacheable. Client-side ITL is unusable (~1.8 tok/SSE-delta); numbers are from `/metrics`. |
