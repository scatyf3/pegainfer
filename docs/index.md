...
| `benchmarks/qwen3-4b-dspark-dflash-accept-metrics.md` | DSpark vs DFlash acceptance reproduced from the `vllm:spec_decode_*` counters instead of parsed logs (1x A6000, matched `deepseek-ai/*_qwen3_4b_block7` at K=7). DFlash reproduces tightly (2.36 mean accepted draft in two runs vs published 2.30); DSpark lands 7-9% high; `random`'s published DSpark loss reproduces once sampling is matched. `sharegpt` stays ~20% high even with prompt count and seed aligned — the open item. Also the first execution of issue #604's validation: counters equal `dflash_lane.rs`'s accept trace bin-by-bin. |
