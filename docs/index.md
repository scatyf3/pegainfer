...
| `benchmarks/toolcall-constrained-decoding.md` | BFCL tool-call `tool_choice=required` vs `auto` on vLLM 0.21/Qwen3-0.6B, **5 runs/cell**. Throughput cost `-11~21%` req/s is steady-state and cache-independent: output inflation (`+7~17%` tokens) + per-token mask (TPOT `+3~11%`). Grammar compile (`~8–12 ms`/schema) is a one-time cold-cache cost vLLM **already caches** — not concurrency-dependent (this corrects the old single-run "TTFT +60% at c=1" framing, which was cache ordering). Client-side ITL unusable (~1.8 tok/SSE-delta); numbers from `/metrics`. |
