...
| `models/qwen35/chunked-prefill.md` | Issue #375: scheduler-level chunked prefill. Per-step prefill **token budget** (`OPENINFER_QWEN35_PREFILL_BUDGET`, default 1024) slices a long prompt across steps and services decode between chunks, killing the unified-step ITL stall — no new kernel (reuses `prefill_forward` continuation). Admission reserves slots/pages for in-flight prefills. Green at the 1024 default and budget 4. Next: part-2 mixed-load ITL benchmark. |
